...
export const navSections = [
  ...
  },
  {
    ...
    items: [
      ...
      { to: '/support',   icon: 'fa-headset',  label: 'Тех. поддержка', access: 'support' },
    ],
  },
  {
    key: 'finance',
...
/**
 * Полный список ссылок c подписями — для выбора стартовой страницы и
 * закрепления пунктов в сайдбаре (поиск по `to`).
 */
export const navAllLinks = [
  ...navPinned.map(p => ({ to: p.to, label: p.label, icon: p.icon, access: p.access })),
  ...navSections.flatMap(s => s.items.map(i => ({
    to: i.to, label: i.label, icon: i.icon, access: i.access,
  }))),
]

export const findNavLink = (to) => navAllLinks.find(l => l.to === to) || null
