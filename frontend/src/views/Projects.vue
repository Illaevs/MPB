<template>
  <div class="projects-view">
    ...
    <section class="projects-toolbar-compact">
      ...
      <div v-if="viewMode === 'table'" class="projects-col-menu">
        <button
          type="button"
          class="projects-toolbar-compact__clear"
        ...
        <transition name="pop">
          <div
            v-if="colMenuOpen"
            class="projects-col-menu__pop"
            v-click-outside="() => colMenuOpen = false"
          >
            <label
              v-for="col in PROJECT_COLUMNS"
              :key="col.key"
              class="projects-col-menu__item"
            >
              <input
                type="checkbox"
        ...
        <table class="table">
          <thead>
            <tr>
              ...
              <th v-show="colVisible('value')">Договорная стоимость</th>
          ...
          <tbody>
            ...
            >
              ...
              <td v-show="colVisible('value')" style="font-family: monospace; font-size: 0.9rem;">
              ...
              <td>
                <div class="d-flex gap-2 justify-end">
                  <button
                    ...
                    :title="isProjectPinned(project) ? 'Открепить из избранного' : 'Закрепить в избранном'"
                    :style="isProjectPinned(project) ? 'color: var(--color-primary);' : ''"
...
const PROJECT_COLUMNS = [
  ...
  { key: 'value', label: 'Договорная стоимость' },
]
...
export default {
  ...
  setup() {
    ...
    // Сохранённое представление таблицы (видимость колонок) —
    // гибрид localStorage + бэкенд через useUiPreferences.
    const { getTableView, setTableView, isPinned, togglePinnedEntity } = useUiPreferences()
    const projectPinPath = (p) => (p?.id ? `/projects/${p.id}` : '')
    const isProjectPinned = (p) => isPinned(projectPinPath(p))
    ...
    const savedView = getTableView('projects')
    ...
    const toggleColumn = (key) => {
      ...
      setTableView('projects', { hiddenCols: hiddenCols.value })
    }
...
.projects-col-menu__item:hover {
  background: var(--color-surface-3);
}

.projects-col-menu__item input {
  cursor: pointer;
}
</style>
