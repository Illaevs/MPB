              ...
              <template v-for="group in groupedSectionRows" :key="group.key">
                ...
                <tr v-for="row in group.rows" :key="row.key">
                  ...
                  <td
                    ...
                    :class="{ 'access-cell--na': isColumnDisabled(row.key, column.key) }"
                  >
                    <span v-if="isColumnDisabled(row.key, column.key)" class="access-na" title="Для этого раздела нет понятия «свои» (нет владельца записи) — действует как «все».">—</span>
                    <label v-else class="access-check" :title="permissionTitle(column.key)">
                      <input
                        type="checkbox"
    ...
    >
      <div class="access-role-form">
        ...
        <label class="access-subtree-toggle">
          <input type="checkbox" v-model="roleForm.subtree_scope" />
          <span>
            Расширять «свои» на подчинённых (оргструктура)
...
const PERMISSION_COLUMNS = [
  ...
  { key: 'edit_all', label: 'Редакт. всех' },
  { key: 'edit_assigned', label: 'Редакт. своих' }
]

// Должно совпадать с backend app/services/ownership.py OWNABLE_SECTIONS.
...
export default {
  ...
  setup() {
    ...
    const ensurePermission = (section) => {
      const current = permissions.value[section] || EMPTY_PERM()
      return {
        read_all: Boolean(current.read_all),
        read_assigned: Boolean(current.read_assigned),
        edit_all: Boolean(current.edit_all),
        edit_assigned: Boolean(current.edit_assigned)
      }
    }
      ...
      try {
        const payload = sections.value.map((section) => {
          ...
          return {
            ...
            read_all: Boolean(p.read_all) || editAll,
            read_assigned: Boolean(p.read_assigned) || editAssigned,
            edit_all: editAll,
    ...
    const toggleMatrixPermission = (section, column, value) => {
      ...
      next[column] = value
      // Авто-импликация: нельзя редактировать то, что не видишь.
      if (column === 'edit_all' && value) next.read_all = true
      else if (column === 'edit_assigned' && value) next.read_assigned = true
    ...
    const grantAllPermissions = () => {
      const next = {}
      sections.value.forEach((section) => {
        const ownable = OWNABLE_SECTIONS.has(section)
        next[section] = {
          read_all: true,
          read_assigned: ownable,
          edit_all: true,
          edit_assigned: ownable
        }
      })
    ...
    const permissionTitle = (column) => {
      switch (column) {
        case 'read_all': return 'Чтение всех записей раздела.'
        case 'read_assigned': return 'Чтение только своих записей (по владельцу). Для разделов без понятия «свои» недоступно.'
        case 'edit_all': return 'Редактирование/удаление любых записей раздела. Автоматически включает «Чтение всех».'
        case 'edit_assigned': return 'Редактирование только своих записей. Автоматически включает «Чтение своих». Для разделов без понятия «свои» недоступно.'
        default: return ''
      }
    }
...
.access-cell--na {
  background: rgba(125, 133, 146, 0.06);
}

.access-na {
  display: inline-block;
  color: var(--color-text-subtle, #9aa3b2);
  font-weight: 700;
  user-select: none;
}
...
.access-subtree-toggle input {
  margin-top: 2px;
}
.access-subtree-toggle small {
  display: block;
