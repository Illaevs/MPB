        ...
        <div class="more-wrap">
          ...
          <transition name="pop">
            <div v-if="moreMenuOpen" class="more-menu" v-click-outside="() => moreMenuOpen = false">
              ...
              <label
                v-for="col in CONTRACT_COLUMNS"
                :key="col.key"
                class="more-menu-item more-menu-item--check"
                @click.stop
              >
                <input
                  type="checkbox"
        ...
        <table class="contracts-table">
          <thead>
            <tr>
              ...
              <th v-show="colVisible('contract_number')" class="sortable" @click="toggleSort('contract_number')">
                <span>Номер</span>
                <i v-if="sortBy === 'contract_number'" class="fas" :class="sortDir === 'asc' ? 'fa-arrow-up-short-wide' : 'fa-arrow-down-wide-short'"></i>
              </th>
              <th v-show="colVisible('contract_date')" class="sortable" @click="toggleSort('contract_date')">
                <span>Дата</span>
                <i v-if="sortBy === 'contract_date'" class="fas" :class="sortDir === 'asc' ? 'fa-arrow-up-short-wide' : 'fa-arrow-down-wide-short'"></i>
              </th>
              <th v-show="colVisible('status')" class="sortable" @click="toggleSort('status')">
                <span>Статус</span>
                <i v-if="sortBy === 'status'" class="fas" :class="sortDir === 'asc' ? 'fa-arrow-up-short-wide' : 'fa-arrow-down-wide-short'"></i>
              </th>
              <th v-show="colVisible('amount')" class="text-right sortable" @click="toggleSort('amount')">
                <span>Сумма</span>
                <i v-if="sortBy === 'amount'" class="fas" :class="sortDir === 'asc' ? 'fa-arrow-up-short-wide' : 'fa-arrow-down-wide-short'"></i>
              </th>
              <th v-show="colVisible('type')">Тип</th>
              <th v-show="colVisible('customer')">Заказчик</th>
              <th v-show="colVisible('executor')">Исполнитель</th>
              <th v-show="colVisible('deal')">Сделка</th>
          ...
          <tbody>
            ...
            >
              ...
              <td v-show="colVisible('contract_number')" class="contracts-num">
                <span class="contracts-num__primary">{{ contract.contract_number }}</span>
              </td>
              <td v-show="colVisible('contract_date')" class="contracts-date">{{ formatDate(contract.contract_date) }}</td>
              <td v-show="colVisible('status')" @click.stop>
              ...
              <td v-show="colVisible('amount')" class="text-right contracts-amount">{{ formatAmount(contract.amount) }}</td>
              <td v-show="colVisible('type')">
                <span class="type-badge" :class="`type-${contract.contract_type || 'other'}`">
                  <span class="type-dot"></span>{{ getTypeText(contract.contract_type) }}
                </span>
              </td>
              <td v-show="colVisible('customer')" :title="getCompanyName(contract.customer_id)">
                <router-link
              ...
              <td v-show="colVisible('executor')" :title="getCompanyName(contract.executor_id)">
                <router-link
              ...
              <td v-show="colVisible('deal')" @click.stop>
    ...
    <Teleport to="body">
      <transition name="pop">
        ...
        >
          ...
          <button type="button" class="more-menu-item" @click="togglePinContract(rowMenuTarget); openRowMenuId = null">
            <i class="fas fa-thumbtack mr-2"></i>
            {{ isContractPinned(rowMenuTarget) ? 'Открепить из избранного' : 'Закрепить в избранном' }}
          </button>
          <div class="more-menu-divider"></div>
...
export default {
  ...
  setup() {
    ...
    // синхронизируется через useUiPreferences (гибрид localStorage + бэкенд).
    const { getTableView, setTableView, isPinned, togglePinnedEntity } = useUiPreferences()
    const contractPinPath = (c) => (c?.id ? `/contracts/${c.id}` : '')
    const isContractPinned = (c) => isPinned(contractPinPath(c))
    const togglePinContract = (c) => {
      if (!c?.id) return
      togglePinnedEntity(contractPinPath(c), {
        label: c.contract_number ? `№ ${c.contract_number}` : 'Договор',
        icon: 'fa-file-contract',
        type: 'contract',
      })
    }
    const savedView = getTableView('contracts')
    const hiddenCols = ref(Array.isArray(savedView.hiddenCols) ? [...savedView.hiddenCols] : [])
    if (typeof savedView.sortBy === 'string') sortBy.value = savedView.sortBy
    if (savedView.sortDir === 'asc' || savedView.sortDir === 'desc') sortDir.value = savedView.sortDir

    const persistTableView = () => {
      setTableView('contracts', {
    ...
    const toggleColumn = (key) => {
      hiddenCols.value = colVisible(key)
        ? [...hiddenCols.value, key]
        : hiddenCols.value.filter((k) => k !== key)
      persistTableView()
    }
    watch([sortBy, sortDir], persistTableView)
    ...
    return {
      ...
      // constants
      CONTRACT_TYPES, STATUS_OPTIONS, CONTRACT_COLUMNS,
      // table view
      colVisible, toggleColumn,
      // pin entity
      isContractPinned, togglePinContract,
      // helpers
...
.more-menu-item--check { gap: 8px; cursor: pointer; }
.more-menu-item--check input { cursor: pointer; }
...
@media (max-width: 600px) {
  /* Поиск — на всю ширину, контролы тулбара с нормальными отступами. */
  .toolbar-row { gap: 8px; row-gap: 10px; }
  .contracts-search { flex: 1 1 100%; margin-left: 0; order: 10; }
}
...
/* Телефон (≤600): строки реестра → карточки. Подпись поля — НАД значением
   (через CSS по nth-child); значения слева, текут естественно — составные
   ячейки «аватар+имя»/бейджи не разъезжаются. Кнопка «⋮» — в правом
   верхнем углу карточки. Разметку td не трогаем. */
@media (max-width: 600px) {
  .contracts-table,
  ...
  .contracts-table td.actions-cell {
    position: absolute;
    top: 6px;
    right: 4px;
    padding: 0;
  }
  .contracts-table td.actions-cell::before { display: none; }
  .contracts-table td .contracts-company-cell { display: inline-flex; max-width: 100%; min-width: 0; }
  .contracts-table td .contracts-company-cell__name { overflow: hidden; text-overflow: ellipsis; }
}
</style>
