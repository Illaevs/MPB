...
export default {
  ...
  setup(props, { emit }) {
    ...
    // M5: sanitize before writing to innerHTML — modelValue originates from
    // server-stored template/body HTML and must not execute event handlers.
    ...
    const writeHtmlIfChanged = (html) => {
      ...
      if (rootRef.value.innerHTML === safe) return
      rootRef.value.innerHTML = safe
    }
