...
// Section-level edit hint for showing/hiding edit & delete controls.
// Final authority is the backend (record-level "own" check); this only
// drives UI affordances.
export const canEditSection = (section, permissionsOverride = null) => {
  const permissions = permissionsOverride || getStoredPermissions()
  if (permissions?.__superuser__?.edit_all) return true
  const permission = permissions?.[section]
  return !!(permission && (permission.edit_all || permission.edit_assigned))
}
