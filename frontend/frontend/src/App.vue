...
export default {
  ...
  setup() {
    ...
    const updateCurrentTime = () => {
      currentTimeLabel.value = new Intl.DateTimeFormat('ru-RU', {
        hour: '2-digit',
        minute: '2-digit'
      }).format(new Date())
    }
    ...
    const applyBodyBackground = (url) => {
      ...
      const currentToken = ++wallpaperApplyToken
      if (!resolvedUrl) {
      ...
      probe.onload = () => {
        if (currentToken === wallpaperApplyToken) {
    ...
    onMounted(() => {
      ...
      applyBodyBackground(authStore.user?.wallpaper_url || '')
      updateCurrentTime()
    ...
    watch(activeUser, (value) => {
      applyBodyBackground(value?.wallpaper_url || '')
