...
export function useTasksState() {
  ...
  const openBulkAssign = () => { bulkAssignUserId.value = ''; bulkAssignOpen.value = true }
  const closeBulkAssign = () => { bulkAssignOpen.value = false }
  const bulkAssign = async () => {
    const userId = bulkAssignUserId.value || null
    const ids = Array.from(selectedTaskIds.value)
    if (!ids.length) return
    bulkBusy.value = true
    let failed = 0
    ...
    ids.forEach((id) => {
      const idx = tasks.value.findIndex(t => String(t.id) === id)
      if (idx >= 0) { original.set(id, { ...tasks.value[idx] }); tasks.value[idx] = { ...tasks.value[idx], assigned_to_user_id: userId } }
    })
    const settle = await Promise.allSettled(ids.map(id => apiTasks.update(id, { assigned_to_user_id: userId })))
    settle.forEach((res, i) => {
      if (res.status === 'rejected') {
        failed += 1
        const id = ids[i]
    ...
    clearTaskSelection()
    await loadTasks()
    if (!failed) toastSuccess(`Назначено: ${ids.length}`)
    else toastWarning(`Готово: ${ids.length - failed}, ошибок: ${failed}`)
  }
  ...
  // Auto-draft: silently create a draft when title is non-trivial
  const triggerAutoDraftIfNeeded = () => {
    if (autoDraftTimer) clearTimeout(autoDraftTimer)
    if (!showCreateModal.value) return
    if (isEditing.value) return
    if (autoDraftFired) return
    const title = String(taskForm.value?.title || '').trim()
    if (title.length < 3) return
    autoDraftTimer = setTimeout(async () => {
      if (!showCreateModal.value || isEditing.value || autoDraftFired) return
      const titleNow = String(taskForm.value?.title || '').trim()
      if (titleNow.length < 3) return
      autoDraftFired = true
      try {
        const payload = { title: titleNow, status: 'new', priority: taskForm.value.priority || 'normal' }
        const data = await apiTasks.create(payload)
        if (data?.id && showCreateModal.value && !isEditing.value) {
          taskForm.value = { ...taskForm.value, id: data.id }
          isEditing.value = true
          tasks.value = [data, ...tasks.value]
          taskPagination.value.total = (taskPagination.value.total || 0) + 1
          syncMatrixColumns()
        }
      } catch (err) {
        autoDraftFired = false
  ...
  return {
    ...
    bulkBusy, bulkSetStatus, bulkAssign, bulkAssignOpen, bulkAssignUserId,
    openBulkAssign, closeBulkAssign, bulkDelete,
    // aside / draft
