  ...
  <div class="users-view">
    ...
    <div class="users-list-shell">
      <div v-if="loading" class="users-list users-list--loading">
        <SkeletonLoader v-for="i in 8" :key="i" height="104px" />
      </div>
...
.users-view {
  ...
  gap: var(--space-5);
  padding: var(--space-5);
  background: linear-gradient(180deg, var(--color-surface-3) 0%, var(--color-bg) 100%);
...
.user-card__main {
  display: grid;
  grid-template-columns: minmax(260px, 1.35fr) minmax(150px, 0.75fr) minmax(220px, 1fr) minmax(220px, 1fr) minmax(220px, 1fr);
  gap: var(--space-4);
  align-items: center;
  min-width: 0;
}
