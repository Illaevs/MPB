<template>
  <div class="tasks-view h-100 d-flex flex-column p-3">
    ...
    <TaskDrawer v-if="useTaskDrawer" :state="state" />
    <TaskEditorModal v-else :state="state" />
...
import TaskEditorModal from './tasks/parts/TaskEditorModal.vue'
...
export default {
  ...
  components: {
    ...
    TaskEditorModal, TaskDrawer, TaskModals
  },
  setup() {
    const state = useTasksState()
    // Feature flag: opt-in to the new wide side-drawer editor.
    // Toggle in DevTools: localStorage.setItem('useTaskDrawer', '1'); location.reload()
    const useTaskDrawer = (() => {
      try { return localStorage.getItem('useTaskDrawer') === '1' } catch { return false }
    })()
    return { state, useTaskDrawer }
  }
}
</script>
<style>
