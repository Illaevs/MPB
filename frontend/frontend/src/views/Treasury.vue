<template>
  <div class="treasury-view h-100 d-flex flex-column p-2">
    ...
    <div class="card p-2 mb-3 flex-shrink-0">
       <div class="d-grid gap-2" style="grid-template-columns: 2fr 1fr 1fr 1fr 1fr 1fr 80px;">
    ...
    <!-- Bulk Actions Bar -->
    <div v-if="selectedIds.length" class="card p-2 mb-2 bg-primary-subtle d-flex align-center gap-3">
       <span class="fw-500">Выбрано: {{ selectedIds.length }}</span>
       <select v-model="bulkAction" class="form-select form-select-sm" style="width: 200px;">
       ...
       <CategorySmartSelect
          ...
          style="width: 300px;"
       />
       <select v-if="bulkAction === 'calc_type'" v-model="bulkCalcType" class="form-select form-select-sm" style="width: 150px;">
          <option value="">Расчёт...</option>
          <option value="vtb">ВТБ</option>
          <option value="material">Матер.</option>
          <option value="other">Иной</option>
       </select>
       <button class="btn btn-sm btn-primary" @click="applyBulkAction" :disabled="!bulkAction || bulkLoading">
          <i v-if="bulkLoading" class="fas fa-spinner fa-spin mr-1"></i> Применить
       </button>
       <button class="btn btn-sm btn-text" @click="clearSelection">Отмена</button>
    </div>
