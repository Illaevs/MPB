...
# EB v2: inbound интеграции от внешних вендоров (HMAC + IP allow-list).
...
# Step 0 search: глобальный полнотекстовый поиск (FTS5 + ACL).
from app.routers.search import router as search_router
# Step 0 search: импорт регистрирует in-process indexer-hook на emit_event
# через `register_after_emit_hook`. Сторонний эффект, не используем напрямую.
...
app.add_middleware(
    ...
    open_prefixes=(
        ...
        # EB v2 inbound integrations: аутентификация по HMAC-подписи
        # внутри каждого приёмника + IP allow-list на nginx; cookie-auth/CSRF
        # не нужны (это внешние клиенты — diadoc / банк / 1С / etc).
        "/api/v1/integrations/",
    ),
)
...
# EB v2: inbound интеграции — приёмники вебхуков от внешних вендоров.
# Без cookie-auth (open_prefix выше); HMAC и IP-allow внутри каждого роутера.
...
# Step 0 search: глобальный поиск. Доступен любому авторизованному;
# per-section ACL применяется внутри роутера.
...
async def bootstrap_chat():
    ...
    # EB v2: автодискавер in-process @on-обработчиков в app/event_handlers/.
    # Discovery идёт ПОСЛЕ всех ensure_* — чтобы хендлеры могли
    # безопасно ссылаться на готовые таблицы.
    # search_indexer.py остаётся отдельно — регистрируется через
    # `register_after_emit_hook` (V1 хук), а не @on-декоратор.
    try:
        ...
        _logging.getLogger("uvicorn.error").exception(
            "event_dispatcher discovery failed: %s", exc,
        )
