...
async def _sync_task_watchers(db: AsyncSession, task_id: str, ids: Optional[List[str]]) -> None:
...
async def create_task(
    ...
    if task.watcher_ids is not None:
        await _sync_task_watchers(db, str(db_task.id), task.watcher_ids)
...
async def update_task(
    ...
    if incoming_watcher_ids is not None:
        await _sync_task_watchers(db, str(task.id), [str(x) for x in incoming_watcher_ids])
    full_budget_payload = (
        task.budget and task.budget > 0
        and task.category_code and task.payer_id and task.payee_id and task.due_date
    )
