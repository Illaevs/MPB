"""
Gantt service for stage scheduling and dependency propagation.
"""
import uuid
...
from app.models import Stage, StageDependency
...
class GanttService:
    ...
    def add_workdays(start_date: date, workdays: int) -> date:
        ...
        while added_days < workdays:
            ...
            if GanttService.is_workday(current_date):
                added_days += 1
    ...
    @staticmethod
    def resolve_effective_end_date(stage: Optional[Stage], fallback_end: Optional[date] = None) -> Optional[date]:
        if stage is None:
            return fallback_end
        if stage.close_date and (
            bool(getattr(stage, "is_closed", False))
            or getattr(stage, "status", None) == "completed"
        ):
    ...
    async def _resolve_successor_schedule(
        ...
        query = select(StageDependency).where(
            StageDependency.successor_id == successor.id
        ).options(
            selectinload(StageDependency.predecessor)
        )
        ...
        if stale_dependency_ids:
            await db.execute(delete(StageDependency).where(StageDependency.id.in_(stale_dependency_ids)))
    ...
    async def get_stage_with_dependencies(
        ...
        query = select(Stage).where(Stage.id == stage_uuid).options(
            selectinload(Stage.dependencies),
            selectinload(Stage.dependents),
        )
        result = await db.execute(query)
    ...
    async def propagate_dates(
        ...
        await db.execute(
            update(Stage).where(Stage.id == stage_uuid).values(
    ...
    async def _update_successors(
        ...
        query = select(StageDependency).where(
            StageDependency.predecessor_id == predecessor_uuid
        ).options(
            selectinload(StageDependency.successor).selectinload(Stage.dependencies)
        )
        ...
        for dependency in dependencies:
            ...
            if successor is None:
                await db.execute(delete(StageDependency).where(StageDependency.id == dependency.id))
                continue
            ...
            await db.execute(
                update(Stage).where(Stage.id == successor.id).values(
    ...
    @classmethod
    async def get_gantt_tree(cls, deal_id: uuid.UUID, db: AsyncSession) -> Dict[str, Any]:
        deal_uuid = cls._normalize_uuid(deal_id)
        query = select(Stage).where(Stage.deal_id == deal_uuid).options(
            selectinload(Stage.dependencies),
            selectinload(Stage.dependents),
            selectinload(Stage.subcontractor),
        ).order_by(Stage.parent_id, Stage.date_start)
