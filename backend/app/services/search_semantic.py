...
# Порог cosine similarity для semantic-кандидатов перед RRF.
# Кандидаты ниже порога считаются «не семантически близкими» и не идут
# в финальный merge. Без порога на малых корпусах в RRF попадает
# случайный хвост (cosine ≈ 0.1-0.2), который засоряет выдачу
# (типичный кейс: запрос «союз» → в выдаче «гит», «Привет»).
# Значение 0.4 для bge-m3 хорошо отсекает шум: 0.4+ это «что-то общее»,
# 0.6+ «сильная связь», 0.1-0.3 «не связано».
MIN_COSINE = float(os.environ.get("HYBRID_MIN_COSINE", "0.4"))
