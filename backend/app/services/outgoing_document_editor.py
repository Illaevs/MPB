...
def normalize_editor_draft(value: Any, document_kind: str) -> Dict[str, Any]:
    ...
    # template_v2 drafts use a different shape (layout-driven editor):
    # {schema_version: 2, mode: "template_v2", template_id, document_kind,
    #  field_values: {...}, editable_regions: {...}}. The legacy block
    # normaliser below would strip everything except schema_version /
    # document_kind / content, so v2 payloads pass through untouched.
    try:
        schema_version = int(payload.get("schema_version") or 0)
    except (TypeError, ValueError):
        schema_version = 0
    if schema_version >= 2 or str(payload.get("mode") or "") == "template_v2":
        return {
            ...
            "editable_regions": payload.get("editable_regions") if isinstance(payload.get("editable_regions"), dict) else {},
        }
...
def validate_editor_draft(draft: Any, document_kind: str) -> Dict[str, List[str]]:
    ...
    errors: List[str] = []
    warnings: List[str] = []
    # template_v2 drafts are layout-driven, not block-based — the block
    # checks below don't apply.
    if str(normalized.get("mode") or "") == "template_v2":
