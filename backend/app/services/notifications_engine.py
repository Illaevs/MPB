...
async def _data_health_summary_recipients(db: AsyncSession) -> List[User]:
    """Получатели ежедневной сводки контроля данных — только те, кто
    реально может открыть раздел /data-health: суперпользователи (роль
    с `is_system`) и роли с явным read-доступом к секции `data_health`.
    Раньше слалось всем читателям секции `projects` — и сводка приходила
    сотрудникам, у которых раздела data-health вовсе нет."""
    perm_rows = await db.execute(
        select(RolePermission.role_id).where(
            RolePermission.section == "data_health",
            (RolePermission.read_all.is_(True)) | (RolePermission.read_assigned.is_(True)),
        )
    )
    allowed_role_ids = {str(row[0]) for row in perm_rows.all() if row[0]}
    ...
    for user, role in users_q.all():
        if role is not None and bool(getattr(role, "is_system", False)):
            recipients.append(user)
        elif user.role_id is not None and str(user.role_id) in allowed_role_ids:
            recipients.append(user)
    return recipients
