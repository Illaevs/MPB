...
from app.services.user_avatar_bootstrap import avatars_root, wallpapers_root


def _resolve_media_filename(
    ...
    if not media_url:
        return None
    raw_url = str(media_url).split("?", 1)[0]
...
def _resolve_avatar_filename(user_id: Union[str, UUID, None], avatar_url: Optional[str]) -> Optional[str]:
    return _resolve_media_filename(user_id, avatar_url, avatars_root())


def _resolve_wallpaper_filename(user_id: Union[str, UUID, None], wallpaper_url: Optional[str]) -> Optional[str]:
    return _resolve_media_filename(user_id, wallpaper_url, wallpapers_root())
...
class UserResponse(UserBase):
    ...
    @field_serializer("wallpaper_url")
    def serialize_wallpaper_url(self, value):
        filename = _resolve_wallpaper_filename(self.id, value)
        if not filename:
            return None
        return f"/api/v1/users/wallpaper-user/{self.serialize_id(self.id)}"

    @field_serializer("ui_preferences")
    def serialize_ui_preferences(self, value):
