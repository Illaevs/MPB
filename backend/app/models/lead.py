...
class Lead(Base):
    ...
    async def delete(cls, db, lead_id: str) -> bool:
        """Каскадно удалить лид и все зависимые записи.

        На leads.id есть FK из 4 таблиц:
          - lead_activities.lead_id  (NOT NULL) — timeline события лида
          - lead_products.lead_id    (NOT NULL) — товары/услуги КП
          - kp_documents.lead_id     (NOT NULL) — связка КП и его версии
          - tasks.lead_id            (NULLABLE) — задачи, привязанные к лиду

        SQLite FK без `ON DELETE CASCADE` блокирует DELETE leads строкой
        с IntegrityError. Поэтому удаляем зависимости вручную, а tasks
        оставляем — просто обнуляем у них lead_id (задачи живут дальше).
        """
        from sqlalchemy import delete as _delete, update as _update, or_, text
        try:
            lead_uuid = lead_id if isinstance(lead_id, uuid.UUID) else uuid.UUID(str(lead_id))
        except (ValueError, TypeError):
            return False
        lead_id_str = str(lead_uuid)
        lead_id_hex = lead_uuid.hex
        variants = [lead_uuid, lead_id_str, lead_id_hex]

        # 1) Зачистка зависимых строк через raw-DELETE по всем форматам id
        # (исторически lead_id мог писаться и в hex, и в canonical).
        for table in ("lead_activities", "lead_products"):
            await db.execute(
                text(f"DELETE FROM {table} WHERE lead_id IN (:c, :h)"),
                {"c": lead_id_str, "h": lead_id_hex},
            )
        # KP документы и их версии: kp_versions(kp_id) → kp_documents(lead_id).
        # Удаляем версии для всех KP этого лида, затем сами KP.
        await db.execute(
            text(
                "DELETE FROM kp_versions WHERE kp_id IN ("
                "  SELECT id FROM kp_documents WHERE lead_id IN (:c, :h)"
                ")"
            ),
            {"c": lead_id_str, "h": lead_id_hex},
        )
        await db.execute(
            text("DELETE FROM kp_documents WHERE lead_id IN (:c, :h)"),
            {"c": lead_id_str, "h": lead_id_hex},
        )
        # Задачи переживают удаление лида — лишь рвём связь.
        await db.execute(
            text("UPDATE tasks SET lead_id = NULL WHERE lead_id IN (:c, :h)"),
            {"c": lead_id_str, "h": lead_id_hex},
        )

        # 2) Сам лид.
        query = _delete(cls).where(or_(cls.id == lead_uuid, cls.id == lead_id_str, cls.id == lead_id_hex))
        result = await db.execute(query)
        await db.commit()
