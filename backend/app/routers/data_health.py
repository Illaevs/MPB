...
router = APIRouter()
# Public sub-router: для endpoints которые должны быть открыты пользователям
# БЕЗ права на секцию data_health. Используется для read-only метаданных,
# нужных в обычных списках (например /deal-counts — индикатор «здоровья»
# проекта рядом со строкой в каталоге Projects.vue, нужен ГИПам/помощникам).
# Подключается в main.py с require_any_section_access("projects","leads",...).
...
@public_router.get("/deal-counts", response_model=DataHealthDealCountsResponse)
