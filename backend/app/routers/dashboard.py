...
# Незавершённые статусы задач — для счётчиков виджета «Мои задачи».
_OPEN_TASK_STATUSES = ("new", "in_progress", "pending", "deferred")


@router.get("/my-task-counts")
async def my_task_counts(
    db: AsyncSession = Depends(get_db),
    user: User = Depends(CurrentUser),
):
    """Счётчики виджета «Мои задачи» (только незавершённые):
    - doing      — я основной исполнитель (`assigned_to_user_id`)
    - helping    — я соисполнитель (TaskAssignee), но не основной
    - assigned   — я постановщик (`created_by_user_id`)
    - watching   — я наблюдатель (TaskWatcher)
    """
    uid = str(user.id)

    doing = int((await db.execute(
    ...
    # Соисполнитель: есть строка TaskAssignee, но я не основной исполнитель.
    helping = int((await db.execute(
        select(func.count(func.distinct(Task.id)))
        .select_from(Task)
        .join(TaskAssignee, TaskAssignee.task_id == Task.id)
        .where(
            TaskAssignee.user_id == uid,
            (Task.assigned_to_user_id != uid) | (Task.assigned_to_user_id.is_(None)),
            Task.status.in_(_OPEN_TASK_STATUSES),
        )
    )).scalar() or 0)

    watching = int((await db.execute(
        select(func.count(func.distinct(Task.id)))
        .select_from(Task)
        .join(TaskWatcher, TaskWatcher.task_id == Task.id)
        .where(
            TaskWatcher.user_id == uid,
            Task.status.in_(_OPEN_TASK_STATUSES),
        )
    )).scalar() or 0)
